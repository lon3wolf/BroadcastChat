    ...
    class UDPServer
    {
            ...
            try
            {
                ...
                while (Running)
                {
                    ...
                    if (UDPSock.Available > 0)
                    {
                        ...
                        //Debugging only
                        //Debug.Write(Encoding.ASCII.GetString(Buffer));
                        ...
                        else
                        {
                            ...
                            if (Parts[0].Substring(0, 4).Equals("PING"))
                            {
                                ...
                                */
                                ...
                                {
                                    case ChatWindow.NotificationSounds.Asterisk:
                                        {
                                            System.Media.SystemSounds.Asterisk.Play();
                                        }
                                        break;
                                    case ChatWindow.NotificationSounds.Beep:
                                        {
                                            System.Media.SystemSounds.Beep.Play();
                                        }
                                        break;
                                    case ChatWindow.NotificationSounds.Exclamation:
                                        {
                                ...
                                //Perhaps in future we can write them on Standard ear stream, just like stdout : stdEar.write("Ding");
                            }
                            #endregion
        ...
        private void UpdateConversation(string Message)
        {
            if (ChatInterface.Conversation.InvokeRequired)
            {
                ...
                /*
                if (ChatInterface.WindowState == FormWindowState.Minimized)
                {
                    ChatInterface.MessageNotification.ShowBalloonTip(3000,"New XBC Message",Message,ToolTipIcon.Info);
                }
                */
            }
        }
